...
assert.equal(contracts.validate('reversa.benchmark.result/v1', {
  schema: 'reversa.benchmark.result/v1', variant: 'baseline', total: 1, succeeded: 1,
  success_rate: '1', mean_latency_ms: 10, mean_cost: 0, regressions: 0, evidence_authority: false,
}).valid, false, 'strict validator must reject numeric strings');

const fakeAjv = { opts: { strict: true }, compile: () => Object.assign((value) => typeof value?.score === 'number', { errors: [] }) };
const ajvAdapter = createAjvStrictAdapter({ ajv: fakeAjv, schemaDocument: { type: 'object' } });
assert.equal(ajvAdapter.validate({ score: 1 }).valid, true);
assert.equal(ajvAdapter.validate({ score: '1' }).valid, false);
...
const fabric = createExecutionFabric({
  providers: { fake: async (request) => ({ exit_code: 0, stdout: `ran:${request.command}`, stderr: '', artifacts: [] }) },
});
...
assert.throws(() => lab.createCandidate({ localization: { file: 'a.js', symbol: 'a', region: '1:2' }, patch: 'x', validations: { mutation: '0.9' } }), /finite number/i);
const c1 = lab.createCandidate({ localization: { file: 'a.js', symbol: 'a', region: '1:2' }, patch: 'patch-a', validations: { tests: 1, static: 1, mutation: 0.6, regression: 0 } });
const c2 = lab.createCandidate({ localization: { file: 'a.js', symbol: 'a', region: '1:2' }, patch: 'patch-b', validations: { tests: 1, static: 1, mutation: 0.9, regression: 0 } });
assert.equal(lab.rankCandidates([c1, c2])[0].patch, 'patch-b');

const quality = createQualityGateRunner();
const report = quality.evaluate({ tests: { passed: 10, failed: 0 }, static_analysis: { critical: 0, high: 0 }, mutation: { killed: 9, survived: 1 } });
...
assert.throws(() => bench.record({ variant: 'bad', success: true, latency_ms: '100' }), /finite number/i);
...
const gateway = createMcpGateway({ handlers: { 'reversa.inspect_repo': async ({ root }) => ({ root }) } });
...
const durable = createDurableWorkflow({ load: async (id) => checkpoints.get(id) ?? null, save: async (id, state) => checkpoints.set(id, structuredClone(state)) });
let executions = 0;
const workflow = [
...
for (const name of forbidden) assert.equal(Boolean(packageJson.dependencies?.[name]), false, `${name} must remain optional`);
