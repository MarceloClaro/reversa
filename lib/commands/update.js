...
const INSTALL_COMMAND = 'npm exec --yes --package=github:MarceloClaro/reversa -- reversa install';
...
export default async function update(args) {
  ...
  console.log(chalk.bold('\n  Reversa: Update — MarceloClaro independent edition\n'));
  ...
  // A edição MarceloClaro é deliberadamente independente: não consulta npm,
  ...
  console.log(chalk.gray(`  Source package:    v${SOURCE_VERSION} (MarceloClaro/reversa)`));

  if (semver.lt(installedVersion, SOURCE_VERSION)) {
    console.log(chalk.hex('#ffa203')('  A newer MarceloClaro distribution is being applied.\n'));
  } else if (semver.gt(installedVersion, SOURCE_VERSION)) {
    console.log(chalk.yellow('  Installed metadata is newer than this source package; managed files will still be refreshed from the current MarceloClaro distribution.\n'));
  } else {
  ...
  const { confirm } = await inquirer.prompt([{
    ...
    type: 'confirm',
    name: 'confirm',
    message: '\nConfirm update from MarceloClaro/reversa?',
    default: true,
  }]);
  if (!confirm) {
  ...
  const appendedTo = [];
  const updateSpinner = ora({ text: 'Updating agents from local MarceloClaro distribution...', color: 'cyan' }).start();
  ...
  try {
...

    {
      ...
      s.distribution = 'MarceloClaro/reversa';
    ...
    updateSpinner.succeed(chalk.hex('#ffa203')('Update complete from MarceloClaro/reversa.'));
  } catch (err) {
  ...
  console.log(chalk.bold('\n  Distribution policy:'));
  console.log('  Source of truth: MarceloClaro/reversa.');
