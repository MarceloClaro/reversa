...
function score(validations = {}) {
  const tests = Number(validations.tests ?? 0);
  const statik = Number(validations.static ?? 0);
  const mutation = Number(validations.mutation ?? 0);
  const regression = Number(validations.regression ?? 0);
