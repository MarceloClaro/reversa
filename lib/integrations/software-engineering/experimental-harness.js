...
function assertCommitSha(value) {
  assertNonEmptyString(value, 'commit_sha');
  if (!/^[0-9a-f]{40}$/i.test(value)) throw new TypeError('commit_sha must be a 40-character hexadecimal SHA');
  return value;
}
...
function pairedCells(rows) {
  ...
  const variantCount = new Set(rows.map((row) => row.variant)).size;
  if (variantCount < 2) return 0;
  return [...byCell.values()].filter((variants) => variants.size === variantCount).length;
}
...
export function createReversaBenchExperimentalHarness({
  ...
  return Object.freeze({
    ...
    report({ confirmatory = false } = {}) {
      ...
      const paired = pairedCells(selected);
      let blockedReason = null;
      if (confirmatory && !selected.length) blockedReason = 'no non-smoke runs available';
      else if (confirmatory && variantsPresent.length < 2) blockedReason = 'fewer than two variants available';
      else if (confirmatory && paired < 1) blockedReason = 'no paired task-seed cells available';
      const effectiveConfirmatory = confirmatory && blockedReason === null;
      ...
      return Object.freeze({
        ...
        confirmatory_blocked_reason: blockedReason,
        causal_claim: false,
