const SCHEMAS = Object.freeze({
  'reversa.code-intelligence.graph/v1': {
    required: ['schema', 'symbols', 'edges', 'ranking', 'evidence_authority'],
  },
  'reversa.execution.result/v2': {
    required: ['schema', 'execution_id', 'task_id', 'provider', 'status', 'exit_code', 'stdout', 'stderr', 'artifacts', 'evidence_authority'],
  },
  'reversa.repair.candidate/v1': {
    required: ['schema', 'candidate_id', 'localization', 'patch', 'validations', 'score', 'evidence_authority'],
  },
  'reversa.quality.report/v1': {
    required: ['schema', 'tests', 'static_analysis', 'mutation_score', 'passed', 'evidence_authority'],
  },
  'reversa.trace/v1': {
    required: ['schema', 'trace_id', 'spans', 'evidence_authority'],
  },
...
export function validateStrictContract(schemaName, value) {
  ...
  if ('evidence_authority' in value && value.evidence_authority !== false) {
    errors.push('evidence_authority must be false');
  }
...
export function createStrictContractRegistry() {
  return Object.freeze({
    ...
    ajv: Object.freeze({
      ...
      recommendation: 'compile schemas/software-engineering-v5.schema.json with Ajv strict mode when Ajv is available',
    }),
  });
}
