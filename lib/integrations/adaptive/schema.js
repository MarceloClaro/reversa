...
export function validateAcmeExperience(experience) {
  ...
  if (experience.extras?.actual_success !== null
    && experience.extras?.actual_success !== undefined
    && typeof experience.extras.actual_success !== 'boolean') {
    errors.push('extras.actual_success must be boolean or null');
  }
  return result(errors);
}

export function validateOfflineDecisionRecord(record) {
  const errors = [];
  if (!record || typeof record !== 'object') return result(['offline decision must be an object']);
  if (record.schema !== OFFLINE_DECISION_SCHEMA) errors.push('invalid offline decision schema');
  if (typeof record.event_id !== 'string' || !record.event_id.trim()) errors.push('event_id required');
  if (typeof record.decided_at !== 'string' || Number.isNaN(Date.parse(record.decided_at))) errors.push('valid decided_at required');
  if (typeof record.stage !== 'string' || !record.stage.trim()) errors.push('stage required');
  for (const key of ['baseline_action', 'shadow_action', 'executed_action']) {
    if (typeof record[key] !== 'string' || !record[key].trim()) errors.push(`${key} required`);
  }
  if (!inUnitIntervalOrNull(record.policy_confidence)) errors.push('policy_confidence must be in [0,1]');
  if (!Number.isFinite(Number(record.reward)) || Number(record.reward) < -1 || Number(record.reward) > 1) {
    errors.push('reward must be in [-1,1]');
  }
  if (record.actual_success !== null && typeof record.actual_success !== 'boolean') errors.push('actual_success must be boolean or null');
  if (!EPISTEMIC_STATES.includes(record.epistemic_state)) errors.push('invalid epistemic_state');
  if (record.evidence_authority !== false) errors.push('evidence_authority must be false');
  return result(errors);
}

export function validateOfflineEvaluation(evaluation) {
  const errors = [];
  if (!evaluation || typeof evaluation !== 'object') return result(['offline evaluation must be an object']);
  if (evaluation.schema !== OFFLINE_EVALUATION_SCHEMA) errors.push('invalid offline evaluation schema');
  if (!['evaluated', 'insufficient_data'].includes(evaluation.status)) errors.push('invalid evaluation status');
  if (!nonNegativeIntegerOrNull(evaluation.total_records)) errors.push('total_records must be non-negative integer');
  if (evaluation.status === 'evaluated') {
    if (!nonNegativeIntegerOrNull(evaluation.train_records)) errors.push('train_records invalid');
    if (!nonNegativeIntegerOrNull(evaluation.holdout_records)) errors.push('holdout_records invalid');
    if (!evaluation.calibration || typeof evaluation.calibration !== 'object') errors.push('calibration required');
    if (!evaluation.estimate || typeof evaluation.estimate !== 'object') errors.push('estimate required');
    if (!Array.isArray(evaluation.training_matrix)) errors.push('training_matrix required');
    if (!Array.isArray(evaluation.holdout_matrix)) errors.push('holdout_matrix required');
  }
  return result(errors);
}

export function validateAdaptiveReport(report) {
  const errors = [];
  if (!report || typeof report !== 'object') return result(['adaptive report must be an object']);
  if (report.schema !== ADAPTIVE_REPORT_SCHEMA) errors.push('invalid adaptive report schema');
  if (typeof report.generated_at !== 'string' || Number.isNaN(Date.parse(report.generated_at))) errors.push('valid generated_at required');
  if (typeof report.markdown !== 'string' || !report.markdown.trim()) errors.push('markdown required');
  if (typeof report.suggested_path !== 'string' || !report.suggested_path.trim()) errors.push('suggested_path required');
  return result(errors);
}
