...
import { validateAdaptiveReport } from './schema.js';
...
export function buildAdaptiveGovernanceReport({
  ...
  const checked = validateAdaptiveReport(report);
  if (!checked.valid) throw new TypeError(`adaptive report inválido: ${checked.errors.join('; ')}`);
