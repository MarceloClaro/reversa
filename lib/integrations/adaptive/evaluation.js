...
import {
  ...
  validateOfflineDecisionRecord,
  validateOfflineEvaluation,
} from './schema.js';
...
export function createDecisionRecord({
  ...
  const checked = validateOfflineDecisionRecord(record);
  if (!checked.valid) throw new TypeError(`offline decision inválida: ${checked.errors.join('; ')}`);
...
export function evaluateOfflinePolicy(records, {
  ...
  const valid = records.filter((record) => validateOfflineDecisionRecord(record).valid);
  if (valid.length < 2) {
  ...
  const checked = validateOfflineEvaluation(evaluation);
  if (!checked.valid) throw new TypeError(`offline evaluation inválida: ${checked.errors.join('; ')}`);
