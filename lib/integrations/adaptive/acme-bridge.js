...
export function buildAcmeObservation(event) {
  ...
  return Object.freeze({
    ...
    vector: Object.freeze([
      ...
      finite(event.state.feynman_score) / 12,
      ...
      finite(event.state.confidence, 0.5),
      finite(event.state.trust, 0.5),
    ]),
...
export function validateAdaptiveAction(actionId, allowedActions = DEFAULT_ADAPTIVE_ACTIONS) {
  ...
  return Object.freeze({ allowed: true, action_id: actionId, reason: 'allowlisted' });
}
